...
?>
		<center>Thứ tự không xác định</center>
		<style>
...
<div class="container-fluid">
	<div class="row">
        <div class="col-3 border bg-gradient-primary"><span class="">Mã tham chiếu</span></div>
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($code) ? $code : '' ?></span></div>
        <div class="col-3 border bg-gradient-primary"><span class="">Nhà xuất bản</span></div>
            ...
            $status = isset($status) ? $status : '';
                switch($status){
                    ...
                    case 4:
                        ...
                        break;
                    case 5:
                        echo '<span class="badge badge-danger bg-gradient-danger px-3 rounded-pill">Đã huỷ</span>';
                        break;
                    default:
	...
	<div class="text-right">
        <?php if(isset($status) && $status == 0): ?>
		    <button class="btn btn-default bg-gradient-danger text-light btn-sm btn-flat" type="button" id="cancel_order">Hủy đơn hàng</button>
        <?php endif; ?>
...
<script>
    $(function(){
        $('#cancel_order').click(function(){
            _conf("Bạn có chắc chắn hủy đơn hàng này không?","cancel_order",['<?= isset($id) ? $id : '' ?>'])
        })
    })
