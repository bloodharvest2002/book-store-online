    ...
    <form action="" id="product-form">
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="form-group">
                    ...
                    <select type="text" id="category_id" name="category_id" class="form-control form-control-sm form-control-border select2" required>
                        ...
                        <?php
                        $categories = $conn->query("SELECT * FROM `category_list` where delete_flag = 0 and `status` = 1".(isset($category_id) ? " or id = '{$category_id}' " : '')." order by `name` asc ");
                ...
                <div class="form-group">
                    <label for="vendor_id" class="control-label">Nhà sản xuất</label>
                    <select type="text" id="vendor_id" name="vendor_id" class="form-control form-control-sm form-control-border select2" required>
                        <option value="" disabled <?= !isset($vendor_id) ? 'selected' : "" ?>></option>
                        <?php
                        $vendors = $conn->query("SELECT * FROM `vendor_list` where delete_flag = 0 and `status` = 1".(isset($vendor_id) ? " or id = '{$vendor_id}' " : '')." order by `shop_name` asc ");
                        while($row = $vendors->fetch_assoc()):
                            ?>
                            <option value="<?= $row['id'] ?>" <?= isset($vendor_id) && $v == $row['id'] ? 'selected': '' ?>><?= $row['shop_name'] ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div class="form-group">
            ...
            <div class="col-md-6">
                ...
                <div class="form-group">
                    <label for="logo" class="control-label">Ảnh Sản Phẩm</label>
