...
Class Master extends DBConnection {
	...
	function save_shop_type(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Loại nhà xuất bản đã tồn tại.";
		}else{
			if(empty($id)){
			...
			if($save){
				$resp['status'] = 'success';
				if(empty($id))
				$resp['msg'] = " Loại nhà xuất bản mới đã được lưu thành công.";
				else
				$resp['msg'] = " Loại nhà xuất bản được cập nhật thành công.";
			}else{
				$resp['status'] = 'failed';
	...
	function delete_shop_type(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Loại nhà xuất bản đã được xóa thành công.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_category(){
		...
		if($check > 0){
			...
			if($save){
				$resp['status'] = 'success';
				if(empty($id))
				$resp['msg'] = " Danh mục mới đã được lưu thành công.";
				else
				$resp['msg'] = " Danh mục được cập nhật thành công.";
			}else{
				$resp['status'] = 'failed';
				$resp['err'] = $this->conn->error."[{$sql}]";
			}
	...
	function save_product(){
		...
		if($check > 0){
			...
			if($save){
				...
				$resp['status'] = 'success';
				if(empty($id))
					$resp['msg'] = " Sản phẩm mới đã được lưu thành công.";
				else
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					if(!in_array($type,$allowed)){
						$resp['msg']=" Nhưng không thể tải lên hình ảnh do loại tệp không hợp lệ.";
					}else{
						...
						$resp['msg']=" Nhưng không tải được hình ảnh lên không rõ nguyên nhân.";
						}
					}
				...
				$resp['status'] = 'failed';
				if(empty($id))
					$resp['msg'] = " Sản phẩm không thể lưu được.";
				else
					$resp['msg'] = " Sản phẩm không cập nhật được.";
				$resp['err'] = $this->conn->error."[{$sql}]";
			}
		}
	...
	function add_to_cart(){
		...
		if($save){
			$resp['status'] = 'success';
			$resp['msg'] = " Sản phẩm đã được thêm vào giỏ hàng.";
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = " Sản phẩm không được thêm vào giỏ hàng.";
	...
	function update_cart_qty(){
		...
		if($update_cart){
			$resp['status'] = 'success';
			$resp['msg'] = ' Số lượng sản phẩm đã được cập nhật thành công';
		}else{
			$resp['status'] = 'success';
			$resp['msg'] = ' Số lượng sản phẩm không cập nhật được';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_cart(){
		...
		if($del){
			$resp['status'] = 'success';
			$resp['msg'] = " Mục giỏ hàng đã được xóa thành công.";
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = " Mục giỏ hàng không thể xóa được.";
			$resp['error'] = $this->conn->error;
		}
		if($resp['status'] =='success'){
			$this->settings->set_flashdata('success',$resp['msg']);
	...
	function place_order(){
		...
		while($vrow = $vendors->fetch_assoc()):
			...
			$data = "('{$ref_code}','{$this->settings->userdata('id')}','{$vrow['id']}','{$this->conn->real_escape_string($delivery_address)}', '{$payment}') ";
			$sql = "INSERT INTO `order_list` (`code`,`client_id`,`vendor_id`,`delivery_address`, `payment`) VALUES {$data}";
			$save = $this->conn->query($sql);
			if($save){
	...
	function cancel_order(){
		...
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = " Đơn hàng đã được hủy thành công.";
		}else{
			$resp['status'] = 'success';
			$resp['error'] = $this->conn->error;
		}
	...
	function update_status(){
		...
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = " Trạng thái đơn hàng đã được cập nhật thành công.";
		}else{
			$resp['status'] = 'success';
			$resp['msg'] = " Trạng thái đơn hàng không cập nhật được.";
			$resp['error'] = $this->conn->error;
		}
		if($resp['status'] == 'success')
		$this->settings->set_flashdata('success',$resp['msg']);
