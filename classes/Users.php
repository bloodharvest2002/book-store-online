...
Class Users extends DBConnection {
	...
	public function save_vendor(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = " Tên này đã có người dùng";
		}else{
			if(empty($id)){
			...
			if($save){
				...
				if(empty($id)){
					if(strpos($_SERVER['HTTP_REFERER'], 'vendor/register.php') > -1){
						$resp['msg'] = " Tài khoản của bạn đã được đăng ký thành công.";
					}else{
						$resp['msg'] = " Tài khoản nhà xuất bản đã được đăng ký thành công.";
					}
				}else{
					if($this->settings->userdata('login_type') == 2){
						$resp['msg'] = " Chi tiết tài khoản của bạn đã được cập nhật thành công.";
					}else{
						$resp['msg'] = " Chi tiết tài khoản của nhà xuất bản đã được cập nhật thành công.";
					}	
				}
				...
				$resp['status'] = 'failed';
				$resp['msg'] = " Đã xảy ra lỗi khi lưu chi tiết tài khoản.";
	...
	public function delete_vendor(){
		...
		if($qry){
			$this->settings->set_flashdata('success',' Chi tiết nhà xuất bản đã được xóa thành công.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = 'Đã xảy ra lỗi khi xóa dữ liệu.';
			$resp['error'] = $this->conn->error;
		}
		return json_encode($resp);
	}
			...
			if($save){
				...
				if(empty($id)){
					if(strpos($_SERVER['HTTP_REFERER'], 'client/register.php') > -1){
						...
						$resp['msg'] = " Tài khoản của khách hàng đã được đăng ký thành công.";
					}
				}else{
					if($this->settings->userdata('login_type') == 3){
						$resp['msg'] = " Chi tiết tài khoản của bạn đã được cập nhật thành công.";
					}else{
						$resp['msg'] = " Chi tiết tài khoản của khách hàng đã được cập nhật thành công.";
					}	
				}
	...
	public function delete_client(){
		...
		if($qry){
			$this->settings->set_flashdata('success',' Chi tiết khách hàng đã được xóa thành công.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = 'Đã xảy ra lỗi khi xóa dữ liệu.';
